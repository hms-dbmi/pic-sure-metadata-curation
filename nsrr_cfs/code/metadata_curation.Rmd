...
renamed_df <-  data.frame(
                            ...
                            form_name=raw_variables_df$forms,
                            #form_description=,
                            variable_id=raw_variables_df$id,
...
variable_info_df <- data.frame(renamed_df %>%
                         select(form_group,form_name,variable_group_name, variable_group_description, variable_id,variable_name,variable_type) %>%
                         unique())
...
variable_df <- data.frame(variable_info_df %>%
                         select(form_group,form_name,variable_group_name, variable_group_description) %>%
                         unique())
...
for (ind in c(1:nrow(variable_df))) {
  temp_df <- variable_info_df %>%
    filter(form_name == variable_df[ind, "form_name"], variable_group_name == variable_df[ind, "variable_group_name"]) %>%
...
form_descriptions_df <- data.frame(form_name=raw_forms_df$id, form_description=raw_forms_df$display_name)

variable_group_df <- merge(variable_group_df,form_descriptions_df, by="form_name", all = TRUE)
...
for (ind in c(1:nrow(form_df))) {
  ...
  form_df[ind, "form_name"][[1]] <- list(temp_df)
}
```
