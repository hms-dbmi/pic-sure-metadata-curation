...
```{r}
save_label_metadata <- function(df) {
  ...
  for (ind in c(1:nrow(final_df))) {
    label_df <- df %>%
      filter(Variable_Name == final_df[ind, "Variable_Name"]) %>%
      select(Variable_Label_VAR, LABEL, file_data) %>%
      rename(data_dict_label = Variable_Label_VAR,
             sas_label = LABEL)
    label_df <- data.frame(label_df)
    final_df[ind, 'label_data'][[1]] <- list(label_df)
  }
...
```{r}
fourth_fus_i <- save_label_metadata(third_fus_i)
fourth_fus_ii <- save_label_metadata(third_fus_ii)
fourth_phase_iii <- save_label_metadata(third_phase_iii)
```
...
```{r}
final_fus_i <- save_final_level(fourth_fus_i, "BABY HUG Follow-up Study I")
final_fus_ii <- save_final_level(fourth_fus_ii, "BABY HUG Follow-up Study II")
final_phase_iii <- save_final_level(fourth_phase_iii, "BABY HUG Phase III Randomized Clinical Trial")
```
