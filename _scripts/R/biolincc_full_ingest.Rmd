...
Read in the file that contains info for all studies
...
Build a Data Dictionary based off the provided schema file, write the sasfiles to csvs for later viewing
...
for (study_i in (1:length(study_list$phs))) {
  ...
  manifest_path <- list.files(path = "../rawData/",pattern = paste0("^.*\\b", study_list$phs[study_i], "\\b.*$"))[[1]]
...
for (study_i in c(1:length(study_list$ps_abv))) {
  ...
  dd_filepath = paste0("../../",tolower(study_list$ps_abv[study_i]),"/rawData/DataDictionary.csv")
  ...
  # Save file
  my_json <- jsonlite::toJSON(complete_df, pretty=FALSE)
  json_string <- as.character(my_json)
  
  # Rename special fields
  #json_string <- gsub("field_note" , 'Field Note', json_string)
  
  fileConn <- file(paste0("../../",tolower(shortname),"/decoded_data/",shortname,"_metadata.json"))
  writeLines(json_string, fileConn)
  close(fileConn)
  
  if (!dir.exists(paste0("../decodedData/",tolower(shortname),"/output/"))) {
    dir.create(paste0("../decodedData/",tolower(shortname),"/output/"), recursive = TRUE)
  }
  
  metadataCopyFilePath <- paste0("../decodedData/",tolower(shortname),"/output/",tolower(shortname),"_metadata.json")
  if (file.exists(metadataCopyFilePath)) {
    file.remove(metadataCopyFilePath)
  }
  
  file.copy(
    from = paste0("../../",tolower(shortname),"/decoded_data/",shortname,"_metadata.json"),
    to = metadataCopyFilePath,
    overwrite = TRUE
  )
  ...
  for (filepath in data_files_list) {
    ...
    if(TRUE%in%(new_data_df[[subj_id]]%in%tm_trim$c2)){
      ...
      saveXML(doc, file=paste0('../../',tolower(shortname),'/decoded_data/',phs,".v",v,".subjects.p",p,".",shortname,".Subject.data_dict.xml"),prefix = '<?xml-stylesheet type="text/xsl" href="./datadict_v1.xsl"?>')
      saveXML(doc, file=paste0('../../',tolower(shortname),'/rawData/',phs,".v",v,".subjects.p",p,".",shortname,".Subject.data_dict.xml"),prefix = '<?xml-stylesheet type="text/xsl" href="./datadict_v1.xsl"?>')
