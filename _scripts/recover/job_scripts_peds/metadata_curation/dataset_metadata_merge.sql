...
delete from metadata_output.metadata where metadata.concept_path in
(select concept_path from metadata_output.metadata group by concept_path having count(*) > 1)
and display not in
(select min(display) from metadata_output.metadata group by concept_path having count(*) > 1);
DO LANGUAGE Plpgsql $$
    BEGIN
