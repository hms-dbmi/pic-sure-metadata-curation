...
/*alter table dictionary_files.derived_visits
drop column redcap_desc;*/

alter table dictionary_files.derived_visits
add column redcap_desc text;

update dictionary_files.derived_visits set
redcap_desc =
...
update dictionary_files.derived_visits set
redcap_desc =
  regexp_substr(concept_name, '[^\[]*') from input.concepts where field_name != variable and variable = data_field_name
                                      and type ~ 'REDCap';
/*alter table dictionary_files.derived_visits
...
FROM dictionary_files.derived_visits,
LATERAL unnest(regexp_split_to_array(levels, '\n')) AS split_label
where levels is not null and strpos(levels, ':') >0
group by variable
)ini
where ini.variable = derived_visits.variable  and type !~ 'REDCap';
