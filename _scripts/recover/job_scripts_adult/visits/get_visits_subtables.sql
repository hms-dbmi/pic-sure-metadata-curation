CREATE OR REPLACE FUNCTION get_visits_subtables_with_visit_type()
	...
	BEGIN
        ...
        select array_agg(distinct(regexp_replace(lower(trim(visit_type)), '[ \-]', '_', 'g'))) into table_names from input.visits;
        ...
        LOOP
            ...
            table_statement = 'create table output_visits.' || quote_ident(t_name) || ' as
                (select participant_id, '||
                 ...
                 ||
                    ' from input.visits where regexp_replace(lower(trim(visit_type)), ''[ \-]'', ''_'', ''g'') = ' || quote_literal(t_name) || ')';
