...
/*alter table dictionary_files.derived_core
drop column redcap_desc;*/

alter table dictionary_files.derived_core
add column redcap_desc text;

update dictionary_files.derived_core set
redcap_desc =
...
FROM dictionary_files.derived_core,
LATERAL unnest(regexp_split_to_array(levels, '\n')) AS split_label
where levels is not null and strpos(levels, ':') >0
group by variable
)ini
where ini.variable = derived_core.variable  and type !~ 'REDCap';
