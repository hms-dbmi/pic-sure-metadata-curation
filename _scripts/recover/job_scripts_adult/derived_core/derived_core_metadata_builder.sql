...
SELECT meta_utils_id.value                                                        AS dataset_ref,
       colname || meta_utils_suffix.value                                         AS name,
       ...
       JSON_BUILD_OBJECT(
           --metadata key: description
               'description', coalesce(full_desc, '') || 'Derived from biostats_derived_core_proc source tsv.' || CASE
...
FROM input.derived_core
         ...
         LEFT JOIN (SELECT value
                    FROM resources.meta_utils
                    WHERE key = 'dataset_name') AS meta_utils_name ON TRUE
         LEFT JOIN (SELECT value
                    FROM resources.meta_utils
                    WHERE key = 'dataset_suffix') AS meta_utils_suffix ON TRUE
         LEFT JOIN (select coalesce(redcap_desc || ' ', '') || coalesce(description, '') as full_desc, variable
                    from dictionary_files.derived_core) as dict on dict.variable = column_name
         LEFT JOIN (SELECT ARRAY_TO_JSON(ARRAY_AGG(ga4gh_drs_uri))::text AS uri
                    FROM resources.manifest
                    WHERE (file_name ~* 'core_proc')
                      AND (file_name ~* (SELECT value
                                         FROM resources.meta_utils
...
GROUP BY colname, data_type, meta_utils_id.value, meta_utils_name.value,
         meta_utils_suffix.value, drs.uri, dict.full_desc;
