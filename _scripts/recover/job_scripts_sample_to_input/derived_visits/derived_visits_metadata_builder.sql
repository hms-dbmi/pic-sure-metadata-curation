...
from input.derived_visits
         ...
         LEFT JOIN (select coalesce(redcap_desc || ' ', '') || coalesce(description, '') as full_desc, variable
                             from dictionary_files.derived_visits) as dict on dict.variable = column_name or dict.variable || '_2023' = column_name
...
group by colname, data_type, meta_utils_id.value, meta_utils_name.value,meta_utils_suffix.value, drs.uri, infect_yn_curr, visit_month_curr, full_desc;
