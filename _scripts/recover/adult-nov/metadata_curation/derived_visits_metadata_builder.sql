create or replace function get_biostats_derived_visits_metadata()
	returns void as
	$$
...
BEGIN
        drop table if exists processing_metadata.biostats_derived_visits_meta;
        create table if not exists processing_metadata.biostats_derived_visits_meta(varname text, vardesc text, docfile text, type text, values text, comment1 text, max numeric, min numeric, picsure_concept_path text, drs_uri text);

        select array_agg(distinct(redcap_event_name)) into table_names
            from adult.biostats_derived_visits;
        select array_agg(column_name) into column_names from information_schema.columns where table_schema = 'adult' and table_name = 'biostats_derived_visits' and column_name != 'record_id';
 FOR i IN 1 .. array_upper(table_names, 1)
        LOOP
            event_name=table_names[i];
            output_table_name='biostats_derived_visits_' || lower(table_names[i]);
            ...
            LOOP
                ...
                insert_text='insert into processing_metadata.biostats_derived_visits_meta (' ||
                ...
                ''Derived from biostats_derived_visits.tsv.'' as comment1,
                ...
                ''\phs003463\RECOVER_Adult\biostats_derived_visits\'' || lower('''||column_names[j] || column_suffix ||''') || ''\'' as picsure_concept_path,
                ''["drs://dg.4503:dg.4503%2Ffe973c70-8180-46bc-8428-de4e8fa16319"]'' as drs_uri ' ||
                            'from adult.biostats_derived_visits where biostats_derived_visits.redcap_event_name = '''|| event_name || ''' group by redcap_event_name)';
        ...
        update processing_metadata.biostats_derived_visits_meta set vardesc = (varname || vardesc) where vardesc ~ '^( \()';

END
$$ LANGUAGE Plpgsql;
select * from get_biostats_derived_visits_metadata();
