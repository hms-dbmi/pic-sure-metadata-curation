--used to build small sample datasets for testing recover pediatrics dataset in phs003461
...
create table sample.answerdata as (select *
                                   ...
                                   where participant_id in (select participant_id
                                                            ...
                                                            from input.visits
                                                            intersect
                                                            select participant_id from input.fitbit
                                                            intersect
                                                            select record_id from input.derived_core
                                                            limit 100));
...
create table sample.visits as (select *
                                   ...
                                   where participant_id in (select participant_id
                                                            from sample.answerdata));
create table sample.derived_core as (select *
                                   from input.derived_core
                                   where record_id in (select participant_id
                                                            from sample.answerdata));
create table sample.fitbit as (select *
                                   from input.fitbit
                                   where participant_id in (select participant_id
                                                            from sample.answerdata));
