CREATE OR REPLACE FUNCTION get_visits_subtables_with_visit_type()
	...
	BEGIN
        ...
        select array_agg(distinct(regexp_replace(lower(trim(visit_type)), '[ \-]', '_', 'g'))) into table_names from sample.visits;
        ...
        LOOP
            ...
            select array_agg(column_string) into column_names from
                ...
                from information_schema.columns where
                 table_schema = 'sample' and table_name = 'visits' and column_name != 'participant_id')innie;

            table_statement = 'create table output_visits.' || quote_ident(t_name) || ' as
                (select participant_id, '||
                    array_to_string(column_names, ', ')
                 ||
                    ' from sample.visits where regexp_replace(lower(trim(visit_type)), ''[ \-]'', ''_'', ''g'') = ' || quote_literal(t_name) || ')';
