...
# rename source with variable group
source_group_mapping <- data.frame('form_name' = sheets,
                                   'source' = as.character(c(1:30)))

df2 <- left_join(df, source_group_mapping) %>% 
  select(VARNAME, VARDESC, form_name) %>%
