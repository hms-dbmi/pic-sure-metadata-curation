...
raw_dd_df <- read.csv("~/Documents/pic-sure-metadata-curation/dir/input/Data_dictionary.csv")
...
renamed_df <-  data.frame(
                            ...
                            form=str_remove(raw_dd_df$FileName,".csv"),
                            form_name=str_remove(raw_dd_df$FileName,".csv"),
                            form_description= "Form description not provided",
                            variable_id=toupper(raw_dd_df$Variable.Name),
                            variable_name=raw_dd_df$Description,
                            variable_type=raw_dd_df$Type,
                            variable_group_name="All variables",
                            variable_group_description="All variables within the form",
                            variable_description=raw_dd_df$Description
                            )
...
Study
```{r}
complete_df <- data.frame('study_name' = 'A Pilot Trial of Complement Inhibition Using Eculizumab to Overcome Platelet Transfusion Refractoriness in HLA Allo-Immunized Patients',
                       'study' = 'DIR',
                       'study_phs_number' = 'phs003212',
                       'study_url' = 'https://www.ncbi.nlm.nih.gov/projects/gap/cgi-bin/study.cgi?study_id=phs003212.v1.p1')
...
json_string <- gsub("variable_type" , 'Variable Type', json_string)
...
Write File
```{r}
fileConn <- file("../output/dir_metadata.json")
