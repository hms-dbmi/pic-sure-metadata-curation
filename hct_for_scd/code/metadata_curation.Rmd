...
df3 <- left_join(df2, form_mapping) %>%
  ...
  mutate(variable_label_from_data_dictionary = variable_name) %>%  ## will need to adjust variable_name as necessary when sas info is added
...
for(ind in 1:nrow(variable_df)){
  var_df <- df3 %>%
    filter(variable_id == variable_df[ind, 'variable_id'],
           ...
           #data_file_name == variable_df[ind, 'data_file_name']  ## uncomment line when sas info is added
           form_name == variable_df[ind, 'form_name']) %>%
    select(variable_label_from_data_dictionary, variable_hct_status, variable_was_computed, variable_question_number_inform) ## add variable_label_from_data_file when sas info is added
...
```{r}
form_df <- df3 %>% select(form_name, form_id, form_description) %>% unique() ## include data_file_name when sas info is added
